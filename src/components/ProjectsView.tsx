...
import {
  ...
  CalendarDays, DollarSign, User, Tag, FileText, Repeat,
} from "lucide-react";
...
export default function ProjectsView() {
  ...
  // Task form (shared with calendar-style fields)
  ...
  // Project costs from financial entries
  ...
  const saveTask = async () => {
    ...
    // Also create/update calendar event
    if (taskScheduledDate) {
      if (editingTask) {
        // Update existing calendar event if linked
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-72 shrink-0 border-r border-border/30 p-4">
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-xs text-muted-foreground/60 uppercase tracking-wider border-b border-border/20">
                    ...
                    <th className="text-left py-2.5 px-3 w-28">Responsável</th>
                    <th className="text-left py-2.5 px-3 w-28">Data</th>
                    <th className="text-left py-2.5 px-3 w-24">Categoria</th>
                    <th className="text-center py-2.5 px-3 w-16">Status</th>
                    <th className="text-right py-2.5 px-3 w-20">Ações</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tbody>
                      {completedTasks.map((task, idx) => (
                        <TaskRow
                          key={task.id}
                          task={task}
                          index={activeTasks.length + idx + 1}
                          categories={categories}
                          onToggle={toggleComplete}
                          onEdit={openEditTask}
                          onDelete={deleteTask}
      ...
      {/* Task Dialog - unified with calendar-style fields */}
      <Dialog open={taskDialogOpen} onOpenChange={(o) => { setTaskDialogOpen(o); if (!o) resetTaskForm(); }}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Button onClick={saveTask} className="w-full gap-2">
              <Save className="h-4 w-4" /> {editingTask ? "Salvar Alterações" : "Criar Tarefa"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
...
function TaskRow({ task, index, categories, onToggle, onEdit, onDelete }: {
  task: Tables<"tasks">;
  index: number;
  categories: Tables<"categories">[];
  onToggle: (t: Tables<"tasks">) => void;
  onEdit: (t: Tables<"tasks">) => void;
  ...
  return (
    <tr className={cn(
      ...
      <td className="py-2.5 px-3">
        <button onClick={(e) => { e.stopPropagation(); onToggle(task); }} className={cn(
      ...
      <td className="py-2.5 px-3 text-center">
        <span className={cn(
          ...
          task.is_completed ? "bg-success/10 text-success" : "bg-warning/10 text-warning"
        )}>
          {task.is_completed ? "Feito" : "Pendente"}
        </span>
      </td>
      <td className="py-2.5 px-3">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={(e) => { e.stopPropagation(); onEdit(task); }} className="rounded p-1.5 hover:bg-muted">
            <Pencil className="h-3.5 w-3.5 text-muted-foreground" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="rounded p-1.5 hover:bg-destructive/10">
            <Trash2 className="h-3.5 w-3.5 text-destructive/60" />
          </button>
        </div>
      </td>
